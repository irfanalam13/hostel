...
import { FINANCE_SECTIONS } from "../registry";
import type { Money as MoneyValue } from "../types/finance.types";
...
/**
 * Maps every status string used across the finance entities (invoices,
 * payments, fee-assignments, awards, expenses, refunds) to a semantic tone.
 * Anything unmapped falls back to muted.
 */
...
/** Wraps every Finance page: eyebrow + title bar + section pills + content. */
    ...
    <div className="mx-auto max-w-6xl p-4 sm:p-6 space-y-5">
      ...
      <nav className="flex flex-wrap gap-1.5">
        {FINANCE_SECTIONS.map((sec) => {
          const on =
            sec.href === "/finance" ? pathname === "/finance" : pathname.startsWith(sec.href);
          const Icon = sec.icon;
          return (
            <Link
              key={sec.id}
