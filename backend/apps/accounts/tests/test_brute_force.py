...
def test_repeated_failures_lock_the_account(api, user):
    ...
    for _ in range(3):
        api.post(LOGIN, {"username": user.username, "password": "wrong"})

    # After hitting the failure limit, even the CORRECT password is refused.
    blocked = api.post(LOGIN, {"username": user.username, "password": "S3cretPass!"})
    assert blocked.status_code != 200
    assert blocked.status_code in (401, 403, 429)
    ...
    reset()
    assert api.post(LOGIN, {"username": user.username, "password": "S3cretPass!"}).status_code == 200
...
def test_under_limit_does_not_lock(api, user):
    ...
    for _ in range(2):
        api.post(LOGIN, {"username": user.username, "password": "wrong"})
    ok = api.post(LOGIN, {"username": user.username, "password": "S3cretPass!"})
